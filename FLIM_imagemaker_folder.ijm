function countBF(basedir) {
	...
	for (i=0; i<list.length; i++) {
		if (endsWith(list[i], ".ics") && !endsWith(list[i], "/")) {
...
function FLIM_img(img, basedir) {
	...
	//PMID: 28524818
	setBatchMode(true);
	basename = substring(img, 0, indexOf(img, ".ics"));
	BMPFile = basedir + File.separator + basename + ".bmp";
	...
	run("physics");
	//run("Invert LUT");
	setMinAndMax(1.5000, 4.2000); //contrast from 1.6 ns to 3 ns
	//run("RGB Color");
	run("Copy to System");
	...
	var flim = basename+"_composite.tif";
	saveAs("Tiff", basedir + File.separator + "flim" + File.separator + flim);
	close(flim);
}
